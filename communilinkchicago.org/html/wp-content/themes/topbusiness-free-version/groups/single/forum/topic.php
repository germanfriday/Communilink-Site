	...
	if ( bp_has_forum_topic_posts() ) : ?>
		...
		<form action="<?php bp_forum_topic_action(); ?>" method="post" id="forum-topic-form" class="standard-form">
			...
			<div id="topic-meta">
				...
				<h3><?php bp_the_topic_title(); ?> <span class="f11">(<?php bp_the_topic_total_post_count(); ?>)</span></h3>
	
				<?php

					if ( bp_forum_topic_has_tags() ) : ?>
					...
					while ( bp_forum_topic_posts() ) : bp_the_forum_topic_post(); ?>
						...
						<li id="post-<?php bp_the_topic_post_id(); ?>" class="<?php bp_the_topic_post_css_class(); ?>">
							...
							<div class="poster-meta">
								...
								<a href="<?php bp_the_topic_post_poster_link(); ?>"><?php bp_the_topic_post_poster_avatar( 'width=40&height=40' ); ?></a>
			...
			<?php
				...
				if ( ( is_user_logged_in() && 'public' == bp_get_group_status() ) || bp_group_is_member() ) :
					...
					if ( bp_get_the_topic_is_last_page() ) :
						...
						if ( bp_get_the_topic_is_topic_open() && !bp_group_is_user_banned() ) : ?>
							...
							<div id="post-topic-reply">
								...
								<?php
									...
									if ( bp_groups_auto_join() && !bp_group_is_member() ) : ?>
										...
										<p><?php _e( 'You will auto join this group when you reply to this topic.', 'buddypress' ); ?></p><?php
								...
								<div class="submit"><input type="submit" name="submit_reply" id="submit" value="<?php _e( 'Post Reply', 'buddypress' ); ?>" /></div>
						...
						elseif ( !bp_group_is_user_banned() ) : ?>
							...
							<div id="message" class="info"><p><?php _e( 'This topic is closed, replies are no longer accepted.', 'buddypress' ); ?></p></div><?php
		...
		</form><!-- #forum-topic-form --><?php

	else : ?>

		<div id="message" class="info"><p><?php _e( 'There are no posts for this topic.', 'buddypress' ); ?></p></div><?php
