					...
					<?php
						...
						*/
						...
						if ( bp_is_group_creation_step( 'group-details' ) ) :
							...
							<textarea name="group-desc" id="group-desc" aria-required="true"><?php bp_new_group_description(); ?></textarea><?php
						...
						*/
						...
						if ( bp_is_group_creation_step( 'group-settings' ) ) :
							...
							<?php
								...
								wp_nonce_field( 'groups_create_save_group-settings' );
						...
						*/
						...
						if ( bp_is_group_creation_step( 'group-invites' ) ) :
											...
											if ( bp_group_has_invites() ) :
												...
												while ( bp_group_invites() ) : bp_group_the_invite(); ?>
													...
													<li id="<?php bp_group_invite_item_id(); ?>">
														...
														<span class="f120 block ntda"><?php bp_group_invite_user_link(); ?></span>
							...
							wp_nonce_field( 'groups_create_save_group-invites' );
	
							do_action( 'bp_after_group_invites_creation_step' );
	
						endif;
	
						do_action( 'groups_custom_create_steps' ); // Allow plugins to add custom group creation steps
							...
							<div class="submit" id="previous-next">
								...
								<?php
									...
									if ( !bp_is_first_group_creation_step() ) : ?>
										...
										<input type="button" value="<?php _e( 'Back to Previous Step', 'buddypress' ); ?>" id="group-creation-previous" name="previous" onclick="location.href='<?php bp_group_creation_previous_link(); ?>'" /><?php
									...
									if ( !bp_is_last_group_creation_step() && !bp_is_first_group_creation_step() ) : ?>
										...
										<input type="submit" value="<?php _e( 'Next Step', 'buddypress' ); ?>" id="group-creation-next" name="save" /><?php
									...
									if ( bp_is_last_group_creation_step() ) : ?>
										...
										<input type="submit" value="<?php _e( 'Finish', 'buddypress' ); ?>" id="group-creation-finish" name="save" /><?php
