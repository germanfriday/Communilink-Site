				...
				foreach($recentposts as $post) :
					...
					$out .= '<div style="width:100%; height:' . $th . 'px;">';
						...
						$src = ($src) ? $src[0] : $theme_options['img_placeholder'];
							...
							// Display image
							if ($src) : $out .= '<a class="block posts2-img '.$if.'" href="'.get_permalink($post->ID).'"><img src="'.$template_url.'/timthumb.php?src='.$path.'&w='.$tw.'&h='.$th.'&zc=1&q=90'.$c.'" alt="'.get_the_title($post->ID).'" /></a>';
							...
							else : $out .= '<a class="block posts2-img '.$if.'" href="'.get_permalink($post->ID).'"><img src="'.$template_url.'/timthumb.php?src='.get_bloginfo('template_url').'/images/blog-thumb-240x240.jpg&w='.$tw.'&h='.$th.'&zc=1&q=90'.$c.'" alt="no image"/></a>';
						...
						// M e t a s
							...
							$out .= '<div class="posts2-meta ' . $mf . ' lh13" style="width:' . $tw . 'px">';
								...
								// Date
								$out .= $date != 'no' ? '<em class="f11">' . get_the_time( get_option('date_format'), $post->ID ) . '</em><div class="clear h10"><!-- divider --></div>' : '<div class="clear h5"><!-- divider --></div>';
