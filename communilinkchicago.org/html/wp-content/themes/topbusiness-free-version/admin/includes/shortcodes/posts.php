							...
							<div'.$margin.$last.'>';
								...
								if ($t != 'none') :
									...
									$src = $src[0];
										...
										// Display image
										if ($src) : $out .= '<a class="'.$a.'" href="'.get_permalink($post->ID).'" style="width:'.$tw.'px; height:'.$th.'px;"><img src="'.$template_url.'/timthumb.php?src='.$path.'&w='.$tw.'&h='.$th.'&zc=1&q=90'.$c.'" alt="'.get_the_title($post->ID).'" /></a>'; endif;
								...
								if ($date == 'yes') :
									...
									$out .= '<div class="f11"><em>'.get_the_time( get_option('date_format'), $post->ID ).'</em></div>';
								...
								if ($postedin == 'yes') :
									...
									$out .= '<div class="f11">'.__('Posted in','pandathemes').': '.get_the_category_list(', ','', $post_id = $post->ID).'</div>';
								...
								if ($comments == 'yes' && get_comments_number($post->ID) != 0) :
									...
									$out .= '<div class="f11">'.__('Comments','pandathemes').': <a href="'.get_comments_link($post->ID).'">'.get_comments_number($post->ID).'</a></div>';
								...
								if ($tags == 'yes') :
									...
									$out .= '<div class="f11">'.__('Tags','pandathemes').': '.$tag.'</div>';
